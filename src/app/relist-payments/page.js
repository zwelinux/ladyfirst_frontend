  ...
  useEffect(() => {
    ...
    async function loadPayments() {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            typeof data === "string" && data.trim().startsWith("<!DOCTYPE")
              ? "Backend returned HTML instead of JSON for relist payments. Check the Django server error or admin permission."
              : "Failed to load relist payments.",
          );
        }
        ...
        if (active) {
          setError(loadError.message || "Unable to load relist payments.");
        }
      } finally {
        if (active) {
          setLoading(false);
