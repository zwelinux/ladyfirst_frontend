...
export default function ReturnPaymentsPage() {
  ...
  useEffect(() => {
    ...
    async function loadPayments() {
      ...
      try {
        const response = await fetch(buildApiUrl("/admin/return-payments/"), {
          headers: {
        ...
        if (!response.ok) {
          throw new Error(
            typeof data === "string" && data.trim().startsWith("<!DOCTYPE")
              ? "Backend returned HTML instead of JSON for return payments. Check the Django server error or admin permission."
              : "Failed to load return payments.",
          );
        }
        ...
        if (active) {
          setError(loadError.message || "Unable to load return payments.");
        }
      } finally {
        if (active) {
          setLoading(false);
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {payments.map((payment) => (
                      <tr key={payment.id}>
                        ...
                        <td className="px-4 py-4 text-sm text-slate-700">
                          <div>{payment.shipping_method_name || "-"}</div>
                          <div className="text-xs text-slate-500">
                            {payment.address_summary || "-"}
                          </div>
                        </td>
                        <td className="px-4 py-4 text-sm text-slate-700">
                          <div>Delivery: {payment.delivery_fee}</div>
                          <div>Coupon: {payment.coupon_used}</div>
